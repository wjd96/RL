...
class GameACFFNetwork(GameACNetwork):
  def __init__(self,action_size, thread_index,device="/cpu:0"):
    ...
    with tf.device(self._device), tf.variable_scope(scope_name) as scope:
      ...
      self.W_conv3, self.b_conv3 = self._conv_variable([3,3,32,64])
      ...
      # state (input)
      self.s = tf.placeholder("float", [None, 84, 84, 4],name="s")
    
      h_conv1 = tf.nn.relu(self._conv2d(self.s,  self.W_conv1, 4) + self.b_conv1)
      h_conv2 = tf.nn.relu(self._conv2d(h_conv1, self.W_conv2, 2) + self.b_conv2)
      h_conv3 = tf.nn.relu(self._conv2d(h_conv2, self.W_conv3, 1) + self.b_conv3)
      h_conv2_flat = tf.reshape(h_conv3, [-1, 3136])
      h_fc1 = tf.nn.relu(tf.matmul(h_conv2_flat, self.W_fc1) + self.b_fc1)

      # policy (output)
      self.pi = tf.nn.softmax(tf.matmul(h_fc1, self.W_fc2) + self.b_fc2,name="action")
      # value (output)
